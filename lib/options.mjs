...
import { eVal, asVal, asErr, isVal, isErr } from './val_or_error.mjs'
...
function readConfig(fileName, debug) {
  ...
  if (isErr(contents)) {
    return log.exit(`error reading config file "${fileName}": ${contents.getErr().message}`, 1)
  }

  const parsed = parseYaml(contents.getVal())
  if (isErr(parsed)) {
    return log.exit(`error parsing YAML in "${fileName}": ${parsed.getErr().message}`, 1)
  }

  const validated = validateConfig(parsed.getVal(), debug)
  if (isErr(validated)) {
    return log.exit(`config not valid in "${fileName}": ${validated.getErr().message}`, 1)
  }

  return validated.getVal()
}
